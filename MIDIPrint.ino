...
void printQR(const char* text) {
  // Calculer la longueur en lisant depuis PROGMEM
  uint8_t len = 0;
  const char* p = text;
  ...
  // Imprimer depuis PROGMEM
  p = text;
  while (pgm_read_byte(p) != 0) {
    mySerial.write(pgm_read_byte(p));
    p++;
  }
...
void printAsciiFun(const char* txt) {
  ...
  while (pgm_read_byte(p) != 0) {
    ...
    if (c == 0xC3 && pgm_read_byte(p) != 0) {
      uint8_t next = pgm_read_byte(p++);
    ...
    // œ
    else if (c == 0xC5 && pgm_read_byte(p) == 0x93) {
      p++;
      printer.print("oe");
    }

    // æ
    else if (c == 0xC3 && pgm_read_byte(p) == 0xA6) {
      p++;
      printer.print("ae");
    }
...
void handleProgramChange(byte channel, byte program) {
  switch(program) {
    case 1 :  printArtist(&ymnk_); break;
    case 2 :  printArtist(&douran_); break;
    case 3 :  printArtist(&deadChic_); break;
    case 4 :  printArtist(&nouvelleVague_); break;
    case 5 :  printArtist(&delaurentis_); break;
    case 6 :  printArtist(&sonia_); break;
    case 7 :  printArtist(&ilEstVilaine_); break;
    case 8 :  printArtist(&elvett_); break;
    case 9 :  printArtist(&bran_); break;
    case 10 : printArtist(&vss_); break;
    case 11 : printArtist(&prettyInside_); break;
    default : break;
  }
}
